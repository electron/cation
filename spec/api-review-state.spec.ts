...
describe('api review',()=>{
  ...
  it('correctly returns PR ready date for semver-major/semver-minor labels',async ()=>{
      ...
      const expectedDate= new Date(payload.pull_request.created_at.getTime() +  MINIMUM_MINOR_OPEN_TIME).toISOString().split('T')[0];
...
it('should correctly update api review label according to reviews by wg-api',async ()=>{
  ...
  const response = await checkPRReadyForMerge(moctokit,noReviewLabelPayload.pull_request,{approved:[],declined:[" "," "],requestedChanges:[]});
  expect(response).toEqual(undefined);

  // if two or more member of wg-api approved and none of them requested changes
  const response2 = await checkPRReadyForMerge(moctokit,noReviewLabelPayload.pull_request,{approved:[" "," "],declined:[],requestedChanges:[]});
  expect(response2).toEqual(undefined);

  // in any other case
  const response3 = await checkPRReadyForMerge(moctokit,noReviewLabelPayload.pull_request,{approved:[" "],declined:[],requestedChanges:[" "]});
